...
#elif defined(NEWLZ_ARM64) && ! defined(_MSC_VER)
...
static void newLZ_bit_reverse_hufftab(KrakenHuffTab * to_table, const KrakenMSBHuffTab * msbHuff)
{
	SIMPLEPROFILE_SCOPE(huff_table_bitrev);
	RR_COMPILER_ASSERT( ( NEWLZ_HUFF_DECODE_TABLE_SIZE % 64 ) == 0 );
	UINTa e = NEWLZ_HUFF_DECODE_TABLE_SIZE/8;
...
// No good intrinsic for this... sigh.
static inline uint32_t newlz_rev16(uint32_t x)
{
	// rev16 is "reverse 16-bit halfwords inside word", i.e.
	//   ((x >> 8) & 0x00ff00ff) | ((x << 8) & 0xff00ff00)
	uint32_t r;
	__asm__("rev16 %w0,%w1" : "=r"(r) : "r"(x));
	return r;
}
	...
	if (is_huff6)
	{
		...
		if (decodeend1 - decodeptr1 > TRIPLE_DECS_PER_REFILL-1 && in1 - in2 > 8 && in4 - in5 > 8 )
		{
			...
			#define DECONE(strm, outp) \
				...
				bits##strm >>= tabv; \
				/* NOTE(fg): rev16 is used as >>8 that doesn't need a shifter ALU. */ \
				*decodeptr##outp++ = (U8) newlz_rev16(tabv)
