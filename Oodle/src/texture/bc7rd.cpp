	...
	BC7Flags flags)
{
	...
	vector<rrColorBlock4x4> matrix_decoded;
	...
	for (int matrix_chunk_begin = 0; matrix_chunk_begin < matrix_size; matrix_chunk_begin += matrix_chunk_size)
	{
		...
		for LOOP(bi,nb)
		{
			...
			for (int matrix_i = matrix_chunk_begin; matrix_i < matrix_chunk_end; matrix_i++)
			{
				// R loop termination
				//  matrix_rate is in increasing order
				// -> this is not doing much for speed at the moment
				//	max_rate is just too high
				//	but it also doesn't hurt quality
				// @@ REVISIT maybe remove? or try to tighten so that it terminates more?
				if ( matrix_rate[matrix_i] >= st->max_rate )
					break;

				U32 matrix_sad = ColorBlock4x4_ComputeSAD_RGBA(matrix_decoded[matrix_i], colors);
				if ( matrix_sad > st->must_beat_sad )
					continue;
