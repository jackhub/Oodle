...
{
	// Calculate |pixels-pal_entry| as max(pixels-pal_entry, pal_entry-pixels)
	Vec128 diff0 = _mm_sub_epi16(pixels, pal_entry);
	Vec128 diff1 = _mm_sub_epi16(pal_entry, pixels);
	Vec128 absdiff = _mm_max_epi16(diff0, diff1);

	// Max difference is BC4_MAXV - BC4_MINV = 1 << (BC4_SCALE_BITS + 1) = 1 << 13, which is (annoying) _just_ too large
...
static void BC4_FindIndices(const BC4SourceData & values, typename Tblock::Endpoint e0, typename Tblock::Endpoint e1, U32 * pError, BC4Indices * pInds)
{
	...
	// NOTE: this is not a _great_ SSE version, but it's not bad either
	...
	for (int index = 0; index < 8; index++)
	{
		...
		Vec128 v_pal_value = _mm_shuffle_epi32(_mm_unpacklo_epi16(v_palette, v_palette), 0x00); // broadcast first 16-bit value
		...
		v_palette = _mm_srli_si128(v_palette, 2); // shift out lowest palette entry
