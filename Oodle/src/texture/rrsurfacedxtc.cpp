...
rrbool rrSurfaceDXTC_CompressBC2(BlockSurface * to, const BlockSurface * from, rrDXTCLevel dxt1QualityLevel, rrDXTCOptions options )
{
	...
	for LOOP(blocki,from->count)
	{
		...
		rrSingleChannelBlock4x4 alphas;
		RemoveAlpha(&alphas,colors);
	
		rrDXT3Block block;
		DXT3_CompressAlpha(&block.alpha,alphas); // (options & rrDXTCOptions_Dither) != 0 );
		
		rrCompressDXT1Block(&block.dxt1block,colors,(rrDXTCLevel)dxt1QualityLevel,options,true);
		
		rrDXT3Alpha_PutBlock(outPtr,block.alpha);
		outPtr += sizeof(rrDXT3AlphaBlock);
		rrDXT1_PutBlock(outPtr,block.dxt1block);
		outPtr += sizeof(rrDXT1Block);
	}
...
rrbool rrSurfaceDXTC_CompressBC3(BlockSurface * to, const BlockSurface * from, rrDXTCLevel dxt1QualityLevel, rrDXTCOptions options )
{
	...
	for LOOP(blocki,from->count)
	{
		...
		outPtr += sizeof(rrDXT5AlphaBlock);
...
rrbool rrSurfaceDXTC_DecompressBC2(BlockSurface * to, const BlockSurface * from )
{
	...
	for LOOP(blocki,from->count)
	{
		...
		rrDXT3Alpha_GetBlock(inPtr,&block.alpha);	
		inPtr += sizeof(rrDXT3AlphaBlock);
		rrDXT1_GetBlock(inPtr,&block.dxt1block);
		inPtr += sizeof(rrDXT1Block);
		
		rrColorBlock4x4 * pcolors = (rrColorBlock4x4 *)outPtr;
		DXT3_Decompress(pcolors,block);
		SwapRB(pcolors); //DXT1_Decompress wrote BGRA, change to RGBA 
	}
...
rrbool rrSurfaceDXTC_DecompressBC3(BlockSurface * to, const BlockSurface * from )
{
	...
	for LOOP(blocki,from->count)
	{
		...
		rrDXT5Alpha_GetBlock(inPtr,&block.alpha);	
		inPtr += sizeof(rrDXT5AlphaBlock);
		rrDXT1_GetBlock(inPtr,&block.dxt1block);
		inPtr += sizeof(rrDXT1Block);
		
		rrColorBlock4x4 * pcolors = (rrColorBlock4x4 *)outPtr;
		DXT5_Decompress(pcolors,block);
		SwapRB(pcolors); //DXT1_Decompress wrote BGRA, change to RGBA 
	}
